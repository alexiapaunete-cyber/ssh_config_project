...
while read -r linie || [ -n "$linie" ]; do
        ...
        cheie_dictionar="${setare}_${cheie}"

    if [[ -n "${dictionar[$cheie_dictionar]}" ]]; then
        if [[ "${dictionar[$cheie_dictionar]}" == "$valore" ]]; then
                 echo "[DUPLICAT] in sectiunea $setare, optiunea $cheie apare de mai multe ori cu aceeasi valoare!"
        else
                echo"[SUPRASCRIERE] in sectiunea $setare se atribuie mai multe valori diferite!"
        fi
    else
     ...
     case "$cheie" in
                "PermitRootLogin")
                        if [[ "$valoare" != "no" ]]; then
                                echo "$setare: PermitRootLogin este $valoare, se recomanda no  "
                        fi
                        ;;
                "PasswordAuthentication")
                        if [[ "$valoare" != "no" ]]; then
                                 echo " $setare: permite parole, recomandat: no (folositi chei ssh)"
                        fi
                        ;;
                "PermitEmptyPasswords")
                        if [[ "$valoare" != "no" ]]; then
                                 echo "$setare: permite parole goale"
                        fi
                        ;;
                "Port")
                        if [[ "$valoare" == "22" ]]; then
                                echo " $setare: foloseste portul standard 22, schimbati-l pentru a reduce atacurile bot "
                        fi
                        ;;
                "X11Forwarding")
                        if [[ "$valoare" != "no" ]]; then
